WITH competitive_project_contributions AS (
    SELECT pc.id as project, (sub.research_lines ->> 'code')::TEXT as code, (sub.research_lines -> 'contribution')::TEXT::float as contribution
    FROM project_competitive pc
        JOIN (
            SELECT pc.id as project_id, json_array_elements(pc.research_lines) as research_lines
            FROM project_competitive pc
        ) sub ON sub.project_id = pc.id
)

SELECT subquery.group_id, subquery.group_name, sum(subquery.contribution) as total
FROM (
    ...
    SELECT pg.id as parent_group, g.id as group_id, g."name" as group_name, cpc.contribution AS contribution
    FROM "membershipgroup" mg
        JOIN "group" g ON g.id = mg.child_group
        JOIN "group" pg ON pg.id = mg.parent_group
